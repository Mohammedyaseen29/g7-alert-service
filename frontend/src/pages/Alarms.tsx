...
import { BellRing, CircleCheck, Clock3, Download, RefreshCw } from 'lucide-react';
import { api, downloadAlarmHistory } from '../api.js';
...
export function Alarms() {
  ...
  const download = async () => {
    setDownloading(true);
    try { await downloadAlarmHistory(); setError(''); }
    catch { setError('Could not download alarm history. Please try again.'); }
  ...
  return <main className="mx-auto w-full max-w-[1440px] space-y-6 px-4 py-8 sm:px-8 lg:px-10">
    ...
    {!data ? <p role="status" className="py-12 text-center text-slate-500">{error ? 'Alarm data is unavailable.' : 'Loading alarm activity…'}</p> : <>
      ...
      <Card><CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3"><CardTitle className="flex items-center gap-3 text-lg"><Clock3 className="size-5 text-slate-400" />Event history</CardTitle><Button type="button" size="sm" variant="outline" className="gap-2" disabled={downloading} onClick={() => { void download(); }}><Download className="size-4" />{downloading ? 'Downloading…' : 'Download CSV'}</Button></CardHeader><CardContent>
